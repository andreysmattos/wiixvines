    ...
    function index($channel){
        ...
        if(isset($check)){
            ...
            if(auth::check()){
                ...
                return view('channel_view', ['channel' => $channel, 'name' => $name, 'desc' => $desc,'subscribe' => $subscribe, 'ckfollow' => $ckfollow, 'profileimage' => $profileimg, 'idchself' => $idchself, 'default_item' => $defaultitem, 'bg_color' => $bg_color, 'channel_nick' => $channel_nick, 'bg_header' => $bg_header, 'header_text_color' => $header_text_color])->with($data)->with($popular)->with($latest)->with($others);
            }else{

                return view('channel_view', ['channel' => $channel, 'name' => $name, 'desc' => $desc, 'subscribe' => $subscribe, 'profileimage' => $profileimg, 'idchself' => $idchself, 'default_item' => $defaultitem, 'bg_color' => $bg_color, 'channel_nick' => $channel_nick, 'bg_header' => $bg_header, 'header_text_color' => $header_text_color])->with($data)->with($popular)->with($latest)->with($others);
            }                       
        ...
        if($check){
           ...
           //STYLES            
            ...
            $ckfollow = DB::table('subscribes')->where('from_channel_id', '=', $auth_channelid)->where('to_channel_id', '=', $id)->value('id');
                 ...
                 return view('watch', ['name' => $name, 'desc' => $desc, 'channel' => $channel, 'subscribe' => $subscribe, 'ckfollow' => $ckfollow, 'profileimage' => $profileimg, 'default_item' => $defaultitem, 'idchself' => $idchself, 'bg_color' => $bg_color, 'channel_nick' => $channel_nick, 'bg_header' => $bg_header, 'header_text_color' => $header_text_color])->with($vines)->with($popular)->with($latest)->with($others)->with($comments)->with($likes);
