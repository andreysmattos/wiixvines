...
include_once('../php/conn.php');
	...
	$cursos = "SELECT * FROM vines WHERE pvptype LIKE '%$cursos%'";
	$resultado_cursos = mysqli_query($mysqli, $cursos);
	
	if(mysqli_num_rows($resultado_cursos) <= 0){
		echo "<center><span class='border-info border pl-3 pr-3 bg-light mt-3 pt-2 pb-2 font-weight-bold text-dark'>Nenhum vine encontrado...</span></center>";
	}else{
		while($rows = mysqli_fetch_assoc($resultado_cursos)){
						 ?>

				<a href="watch?v=<?= $rows['id'] ?>">
			        <li>
