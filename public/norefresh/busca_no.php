...
include_once('../php/security.php');
	...
	$cursos = "SELECT * FROM vines WHERE title LIKE '%$cursos%'";
	$resultado_cursos = mysqli_query($mysqli, $cursos);
	
	if(mysqli_num_rows($resultado_cursos) <= 0){
		echo "<center><span class='border-info border pl-3 pr-3 pt-2 pb-2 font-weight-bold bg-light mt-3 text-dark'>Nenhum vine encontrado...</span></center>";
	}else{
		while($rows = mysqli_fetch_assoc($resultado_cursos)){
					$idp = $rows['id'];
					$stmt = $mysqli->query("SELECT totalvisit from totalview where page='$idp'");
   				 	$view = $stmt->fetch_assoc();
							  ?>
